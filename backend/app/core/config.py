...
class Settings(BaseSettings):
    ...
    # 认证（警务端）
    # ------------------------------------------------------------------
    # 会话有效期（小时）。8 小时覆盖一个班次：太短会让人反复登录，
    # 太长则一台没人管的机器会一直挂着有效会话。
    auth_session_hours: int = 8
    # 会话 Cookie 名
    auth_cookie_name: str = 'st_session'
    # 是否只经 HTTPS 传输 cookie。本地 http 调试必须为 false，
    # 否则浏览器直接不发送它（表现为"登录成功但立刻又是未登录"）；
    # 生产环境走 HTTPS 时应设为 true。
    auth_cookie_secure: bool = False
    # 新建/seed 警员时的初始口令。
    # **仅供演示与本机使用**，生产应由首次登录强制改密替代。
    auth_default_password: str = 'Traffic@2026'
